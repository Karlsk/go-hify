...
func TestDefaultConfig(t *testing.T) {
	...
	// provider-cache / agent-cache 均显式注册为 30 min。
	if cfg.TTLs[NameProvider] != DefaultTTL || cfg.TTLs[NameAgent] != DefaultTTL {
		t.Fatalf("TTLs = %+v, want both %v", cfg.TTLs, DefaultTTL)
	}
}
