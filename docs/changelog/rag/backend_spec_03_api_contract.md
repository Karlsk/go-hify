...
> 状态：**实施 spec**（2026-09-07），8 篇之 03；决策依据见总览 [backend_module_spec.md](backend_module_spec.md)。前置依赖：01（schema 引用 status 四态等语义）。
> 交付：`internal/rag/api/` 三文件填实（占位文件已在）——**纯契约，api 先行，独立单测**（不依赖 service/store/handler）。

## 1. api/api.go —— `KnowledgeBaseService` 接口（12 方法）

签名恒 `(ctx context.Context, req XxxReq) (*XxxSchema, error)`：

- **KB**：`Create`（embedding 模型预检）/ `Get`（详情含文档计数，Cache-Aside）/ `List`（偏移分页 + name ILIKE 模糊过滤 + 聚合）/ `Update`（name/description/**enabled**（`*bool`，nil 不变）；**embedding_model_id 不可改**——维度冻结，换模型=重建 chunks）/ `Delete`（有文档→InUse）。
- **文档**：`UploadDocument`（落 pending 行后异步 pipeline，返回快照含 file_type/file_size）/ `GetDocument`（含原文）/ `ListDocuments`（KB 下游标分页 keyset id DESC）/ `DeleteDocument`（**软删 + 同事务硬删 chunks**，不变量规则 2）/ `ReindexDocument`（事务内删 chunks + 置 pending + 重跑；进行中→ErrDocumentProcessing）。
...
| `ErrKnowledgeBaseInUse` | 409 | KB 下仍有文档（含软删）挡删 |
