...
## 11. spec 015 增补：运行历史与节点轨迹查询（列表 + 详情抽屉 + 失败高亮 + 截断保真 + 试运行联动 + 存量回归）

交付面：前端 `web/src/views/workflow/WorkflowRunsPanel.vue`（新增「运行历史」卡片，挂 WorkflowDetail.vue 图编排卡片之后）+ `web/src/api/workflow.ts`（`RunSummary` / `RunDetail` / `NodeRunTrack` 类型 + `listWorkflowRuns` / `getWorkflowRun` 方法）；后端 2 条只读 GET 路由（契约见 [api_contract.md](../changelog/workflow/api_contract.md) §1 后两行 + §5「GET runs 运行历史查询」+ §8 `RUN_NOT_FOUND`）。行为依据：[specs/015-workflow-run-history/spec.md](../../specs/015-workflow-run-history/spec.md)（FR-001~FR-006、SC-001~SC-005）与 [quickstart.md](../../specs/015-workflow-run-history/quickstart.md) 场景 D。前置同 §0，追加造数：目标工作流试运行若干次（成功 + 失败各一——失败用指向不可达地址的 api 节点构造，spec 013 同款）；记录多于 20 条时另造 21+ 条验翻页。

### 11.1 场景 A（US1）：列表展示与翻页无重漏（FR-001 / SC-002 / SC-003）

1. 进 workflow 详情页滚动至「运行历史」卡片：挂载自动拉首页，最新在前（刚试运行的记录在顶部）；状态 tag 两态（成功绿 / 失败红）、试运行 tag（warning）、触发来源中文（控制台试运行 / 对话触发 / 子工作流）、耗时「1,234 ms」格式、失败节点与错误摘要空显 —、运行编号等宽
2. 记录 >20 条：点「加载更多」逐页追加；任意深度翻页无重复无遗漏（运行编号序列可核对）；末页按钮隐藏
3. Network 断言（SC-003）：`GET /workflows/{id}/runs` 响应 `data.items` 单条**不含** `input` / `output` 键（摘要面）；`meta` 含 `limit` / `has_more` / `next_cursor`
4. 篡改 cursor（DevTools 改请求 `?cursor=xxx` 重发）→ 400 `VALIDATION_FAILED`（后端行为，自动化已覆盖，此处可选）

### 11.2 场景 B（US2）：失败行可辨 + 详情抽屉轨迹与高亮（FR-003 / SC-001 排障动线）

1. 列表失败行：状态红 tag、失败节点列有值、错误摘要可读——不点开即可初判
2. 点任意行（行指针手势）→ 右侧抽屉「运行 #<id>」打开，按需发 `GET /workflows/{id}/runs/{runId}`；运行级信息 descriptions：状态 / 触发来源 / 试运行 / 总耗时 / 开始时间 / 记录时间；chat 触发且非空时展示关联会话 / 消息；嵌套子运行展示父运行编号
3. 失败运行抽屉：错误文案红色 alert（`error_msg`）+「失败节点：<key>」危险色；节点轨迹表 seq 升序（执行序），**失败节点行（node_key 命中 error_node）危险色背景高亮**——一眼定位
4. 成功运行抽屉：无错误区；轨迹全绿 tag；输入 / 输出区可读
5. 抽屉内轨迹列：序号 / 节点 key / 类型 / 状态 tag / 耗时 / 输入 / 输出（截断 + tooltip 全文，空值显 —）

### 11.3 场景 C（US2）：截断保真与 404 空态（FR-002 / FR-004）

1. 输入 / 输出大文本：pre-wrap 限高滚动、等宽字体；**截断保真文本如实展示不加工不报错**（后端落库的截断标记文本原样渲染）
2. 404 模拟：列表加载后 psql 删除某 run 行（`DELETE FROM workflow_runs WHERE id = N;`）→ 点该行 → 抽屉内空态「该运行记录已不存在」，**无全局 toast**、列表保持
3. 无任何记录的工作流 → 卡片空态「暂无运行记录」+ 头部保留期说明文案（数据窗口语义，FR-004——空 / 缺旧记录为正常态非错误）

### 11.4 场景 D（US3）：试运行联动刷新（FR-005）

1. 详情页试运行一次（任意结果）→ 关闭对话框 → 点运行历史卡片头右侧「刷新」→ 列表整体重置回首页态，顶部出现该记录带试运行标识
2. 点开该记录核对：输入 / 输出 / 轨迹与当次试运行结果一致（关掉对话框后仍可回看——US3 核心场景）
3. 试运行请求进行中连点「刷新」→ loading 防重（只发一次）

### 11.5 存量回归复测（SC-005）

1. 详情页既有区块（基础信息 / Schema / 图编排 / 试运行）与 spec 013/014 后完全一致——运行历史卡片为纯追加，位于图编排卡片之后
2. 本文档 §7.5 EC-12~23、§8.7、§9.8、§10.5 抽测引用复测（编辑 / 试运行 / 双模式编辑器零回归）
3. 既有 8 端点行为与执行落库链路自动化回归已绿（T001/T002 基线对照 + handler 既有用例零改动）

### 11.6 通过标准增补（SC-001~SC-005）

- [ ] 列表 → 点行 → 轨迹 → 失败定位全流程在详情页内闭环（SC-001）（§11.2）
- [ ] 翻页无重漏 + 摘要面无大文本（SC-002 / SC-003）（§11.1）
- [ ] 截断保真 + 404 空态 + 数据窗口文案（SC-001 / FR-004）（§11.3）
- [ ] 试运行 → 刷新 → 顶部记录 → 回看一致（SC-001 / US3）（§11.4）
- [ ] 存量回归复测（SC-005）（§11.5）
- [ ] 双门禁全绿（SC-004 / SC-005）：`cd web && npm run type-check && npm run build`
