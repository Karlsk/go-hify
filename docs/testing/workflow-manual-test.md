...
- 接口前缀：`/api/v1/workflows`（7 端点：POST 创建 / GET 列表 / GET 详情 / PUT 更新 / DELETE 删除 / POST publish / POST disable，受登录中间件保护）
...
```bash
make migrate-up && make migrate-status   # 预期 17 条全部 applied
...
### 12.2 workflow_nodes / workflow_edges 随 CASCADE 清空
